...
#include <linux/regmap_internal.h>
...
static int wm8903_set_pdata_irq_trigger(struct i2c_client *i2c,
					struct wm8903_platform_data *pdata)
{
	struct irq_data *irq_data = get_irq_data(i2c->irq);
	if (!irq_data) {
				      ...
				      const struct i2c_device_id *id)
{
	...
	if (val != 0x8903) {
		printk("Device with ID %x is not a WM8903\n", val);
		ret = -ENODEV;
		goto err;
	}
	...
	if (ret != 0) {
		printk("Failed to read chip revision: %d\n", ret);
		goto err;
	}
